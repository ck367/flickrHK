$(function(){
	...
	// loader image if an album is particularly big
	//jQuery('<img alt="" />').attr('id', 'loader').attr('src', 'https://cdnjs.cloudflare.com/ajax/libs/semantic-ui/0.16.1/images/loader-large.gif').appendTo('#image-container');
	...
	$.getJSON('http://api.flickr.com/services/rest/?&method=flickr.photosets.getPhotos&api_key=' + apiKey + '&photoset_id=72157699047344622&format=json&jsoncallback=?',
		function(data){
	...
	// removes loader image after runtime
	// jQuery('#loader').remove();
				
});

$(document).ready (function() {
    // loops through elements and checks if it includes the text in the input field
	$("#btnSearch").click(function() {
		$('image-container').hide();
		var input = document.getElementById("Search");
		var filter = input.value.toLowerCase();
		var nodes = document.getElementsByClassName('image-container');

		// If the element does match what is in the input field show that element, otherwise hide it.
